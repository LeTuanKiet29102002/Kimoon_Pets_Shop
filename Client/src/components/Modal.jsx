...
const Modal = ({ showModal, setShowModal, type, donhang, setReRenderData, handleClose, showToastFromOut }) => {
    ...
    // Hủy đơn USER
    const HuyDonHang = async ({ tentrangthaidathang, madathang }) => {
        if (tentrangthaidathang === "Chờ xác nhận") {
            const huydonres = await axios.post("http://localhost:3001/api/order/huyDonUser", { madathang });
            if (huydonres.data.message === "Hủy đơn thành công") {
                const dataShow = { message: "Bạn đã hủy đặt mua có mã " + madathang, type: "success" };
                showToastFromOut(dataShow);
                setShowModal(prev => !prev);
                setReRenderData(prev => !prev);
            } else {
                const dataShow = { message: "Có lỗi khi hủy đơn đặt mua có mã " + madathang, type: "danger" };
                showToastFromOut(dataShow);
                setShowModal(prev => !prev);
            }
        } else {
                    ...
                    {showModal ? (
                        <Background ref={modalRef} onClick={closeModal}>
                            <ModalWrapper showModal={showModal} style={{ backgroundImage: `url("https://img.freepik.com/free-vector/alert-safety-background_97886-3460.jpg?w=1060")`, backgroundPosition: `center center`, backgroundRepeat: `no-repeat`, backgroundSize: `cover`, width: `600px`, height: `400px` }} >
                                <ModalContent>
                                    <h2 style={{ textAlign: "center" }}>Bạn muốn hủy đơn đặt mua mã <span style={{ color: `var(--color-primary)` }}>{donhang.madathang}</span> này?</h2>
                                    <p>Bạn chỉ có thể hủy khi đơn đặt mua chưa được duyệt. Bạn có chắc chắn?</p>
                                    <Button>
                                        <ButtonContainer>
                                            <ButtonClick
                                                onClick={() => { HuyDonHang({ tentrangthaidathang: donhang.tentrangthaidathang, madathang: donhang.madathang }) }}
                                            >Đồng ý</ButtonClick>
                                        </ButtonContainer>
                                        <ButtonContainer>
                                            <ButtonClick
