...
router.post("/datMua", async (req, res) => {
  ...
  const sql =
    "insert into dathang (manguoimua, maxa, manhanvien, hotendathang, emaildathang, sodienthoaidathang, diachidathang, ghichudathang, ngaydathang, trangthaidathang, tongtiendathang) values (?, ?, 0, ?, ?, ?, ?, ?, ?, 1, ?);";
  con.query(
    sql,
    [
      req.body.manguoimua,
      req.body.maxa,
      req.body.hotendathang,
      req.body.emaildathang,
      req.body.sdtdathang,
      req.body.diachidathang,
      req.body.ghichudathang,
      ngaydathang,
      req.body.tongtiendathang,
    ],
    (err, result) => {
      if (err) {
        ...
        const sql1 =
          "select * from dathang d join xaphuongthitran x on d.maxa = x.maxa join quanhuyen q on q.maquanhuyen = x.maquanhuyen join tinhthanhpho t on q.mathanhpho = t.mathanhpho where d.ngaydathang = ?;";
        con.query(sql1, [ngaydathang], (err, result1) => {
          if (err) {
            ...
            req.body.giohang.map((thucung, key) => {
              const sql2 =
                "insert into chitietdathang (mathucung, madathang, giachitietdathang, soluongchitietdathang, tongtienchitietdathang) values ( ?, ?, ?, ?, ?)";
              con.query(
                sql2,
                [
                  thucung.data[0].mathucung,
                  result1[0].madathang,
                  thucung.data[0].giamgia,
                  thucung.soluongmua,
                  thucung.data[0].giamgia * thucung.soluongmua,
                ],
                (err, result2) => {
                  if (err) {
                    ...
                    const sql3 =
                      "update thucung set soluong = soluong - ? where mathucung = ?;";
                    con.query(
                      sql3,
                      [thucung.soluongmua, thucung.data[0].mathucung],
                      (err, result3) => {
                        if (err) {
                          console.log("Lỗi khi update số lượng: ", err);
                        } else {
                          console.log(
                            "Update lại số lượng thú cưng trong csdl thành công"
                          );
                        }
                      }
                    );
            ...
            noidung +=
              ...
              result1[0].tenxa +
              ", " +
              result1[0].tenquanhuyen +
              ", " +
              result1[0].tenthanhpho +
              "</p>";
            ...
            var mailOptions = {
              ...
              subject: "Đặt hàng tại Kimoon Pets thành công!",
              html: noidung,
            };
...
router.post("/getDonHangUser", async (req, res) => {
  const sql =
    "select * from dathang d join xaphuongthitran x on d.maxa = x.maxa join quanhuyen q on x.maquanhuyen = q.maquanhuyen join tinhthanhpho t on q.mathanhpho = t.mathanhpho join trangthaidonhang tt on d.trangthaidathang = tt.trangthaidathang where d.manguoimua = ? order by d.ngaydathang desc;";
...
router.post("/huyDonUser", async (req, res) => {
  const sql =
    "update dathang set manhanvien = 0, trangthaidathang = 4 where madathang = ?;";
      ...
      con.query(sql1, [req.body.madathang], (err, result1) => {
        if (err) {
          ...
          result1.map((thucung, key) => {
            ...
            con.query(
              sql2,
              [thucung.soluongchitietdathang, thucung.mathucung],
              (err, result2) => {
                if (err) {
                  console.log(
                    "Lỗi khi update chi tiết thú cưng từ đơn từ chối"
                  );
                } else {
...
router.post("/getDonHang", async (req, res) => {
  const sql =
    "select * from dathang d join xaphuongthitran x on d.maxa = x.maxa join quanhuyen q on x.maquanhuyen = q.maquanhuyen join tinhthanhpho t on q.mathanhpho = t.mathanhpho join trangthaidonhang tt on d.trangthaidathang = tt.trangthaidathang order by d.madathang desc;";
...
router.post("/getChiTietDonHang", async (req, res) => {
  const sql =
    "select * from dathang d join trangthaidonhang tt on d.trangthaidathang = tt.trangthaidathang join nhanvien n on d.manhanvien = n.manhanvien join xaphuongthitran x on d.maxa = x.maxa join quanhuyen q on x.maquanhuyen = q.maquanhuyen join tinhthanhpho t on q.mathanhpho = t.mathanhpho where d.madathang = ?;";
  con.query(sql, [req.body.madathang], (err, result) => {
    if (err) {
...
router.post("/getChiTietThuCung", async (req, res) => {
  const sql =
    "select * from chitietdathang c join thucung t on c.mathucung = t.mathucung WHERE c.madathang = ?;";
  con.query(sql, [req.body.madathang], (err, result) => {
    if (err) {
...
router.post("/findDonHang", async (req, res) => {
  const sql =
    "select * from dathang d join xaphuongthitran x on d.maxa = x.maxa join quanhuyen q on x.maquanhuyen = q.maquanhuyen join tinhthanhpho t on q.mathanhpho = t.mathanhpho join trangthaidonhang tt on d.trangthaidathang = tt.trangthaidathang where d.madathang like concat('%', ?, '%') order by d.madathang desc;";
  con.query(sql, [req.body.madathang], (err, result) => {
    if (err) {
...
router.post("/duyetDon", async (req, res) => {
  const sql =
    "update dathang set trangthaidathang = 2, manhanvien = ? where madathang = ?;";
...
router.post("/tuChoiDon", async (req, res) => {
  const sql =
    "update dathang set trangthaidathang = 4, manhanvien = ? where madathang = ?;";
  con.query(sql, [req.body.manhanvien, req.body.madathang], (err, result) => {
    if (err) {
