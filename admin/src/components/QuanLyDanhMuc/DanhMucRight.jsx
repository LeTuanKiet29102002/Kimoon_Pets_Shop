...
const DanhMucRight = ({ reRenderData, setReRenderData }) => {
  // Thứ ngày tháng
  let today = new Date();
  ...
  // Số lượng danh mục
  const [soLuongDanhMuc, setSoLuongDanhMuc] = useState();
  useEffect(() => {
    const getSoLuongDanhMuc = async () => {
      try {
        const soluongdanhmucres = await axios.get(
          "http://localhost:3001/api/products/getSoLuongDanhMuc"
        );
        setSoLuongDanhMuc(soluongdanhmucres.data[0].soluongdanhmuc);
      } catch (err) {
        console.log("Lỗi: ", err);
      }
    };
  ...
  console.log("Số lượng danh mục: ", soLuongDanhMuc);
    ...
    <Container>
      ...
      <SalesAnalytics>
        ...
        <Item
          ...
          onClick={() => openModal({ type: "themdanhmuc" })}
        >
          <Add />
          <h3>Thêm danh mục</h3>
        </Item>
