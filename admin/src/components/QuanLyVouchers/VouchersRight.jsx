...
const VouchersRight = ({ reRenderData, setReRenderData }) => {
  // Lấy admin từ Redux
  ...
  // Số lượng thú cưng
  const [soLuongVouchers, setSoLuongVouchers] = useState();
  useEffect(() => {
    const getSoLuongThuCungLac = async () => {
      try {
        const soluongvouchersres = await axios.get(
          "http://localhost:3001/api/vouchers/getSoLuongVouchers"
        );
        setSoLuongVouchers(soluongvouchersres.data[0].soluongvouchers);
      } catch (err) {
        console.log("Lỗi: ", err);
      }
    };
  ...
  console.log("Số lượng thú vouchers: ", soLuongVouchers);
          ...
          admin.machucvu === 5 || admin.machucvu === 1 ? (
            <>
              <Item
                ...
                onClick={() => openModal({ type: "themvouchers" })}
              >
                <Add />
                <h3>Thêm vouchers</h3>
              </Item>
